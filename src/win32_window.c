...
//
static const GLFWimage* chooseImage(int count, const GLFWimage* images, int width,
                                    int height) {
...
static HICON createIcon(const GLFWimage* image, int xhot, int yhot, GLFWbool icon) {
    ...
    dc = GetDC(NULL);
    color = CreateDIBSection(dc, (BITMAPINFO*)&bi, DIB_RGB_COLORS, (void**)&target, NULL,
                             (DWORD)0);
    ReleaseDC(NULL, dc);
...
static void maximizeWindowManually(_GLFWwindow* window) {
    ...
    GetMonitorInfoW(MonitorFromWindow(window->win32.handle, MONITOR_DEFAULTTONEAREST),
                    &mi);
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    static RECT borderThickness = {0, 0, 0, 0};
    ...
    if (!window) {
        ...
        switch (uMsg) {
            case WM_CREATE: {
                if (_glfw.hints.window.hasTitlebar) break;

                if (hasThickFrame) {
                    RECT size_rect;
                    GetWindowRect(hWnd, &size_rect);

                    // Inform the application of the frame change to force redrawing with
                    // the new client area that is extended into the title bar
    ...
    switch (uMsg) {
        ...
        case WM_NCCALCSIZE: {
            if (_glfw.hints.window.hasTitlebar || !hasThickFrame || !wParam) break;
            ...
            requested_client_rect->top +=
                frame_y + (window->win32.maximized ? 1 : -1) * padding;
        ...
        case WM_NCHITTEST: {
            ...
            if (hasThickFrame) {
                ...
                if (!window->win32.maximized) {
                    ...
                    if (pt.y < borderThickness.top || pt.y < frame_y + padding)
                        hit |= top;
...
static int createNativeWindow(_GLFWwindow* window, const _GLFWwndconfig* wndconfig,
    ...
    window->win32.handle =
        CreateWindowExW(exStyle, MAKEINTATOM(_glfw.win32.mainWindowClass), wideTitle,
                        style, frameX, frameY, frameWidth, frameHeight,
    ...
    if (IsWindows7OrGreater()) {
        ...
        ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYDATA, MSGFLT_ALLOW,
                                    NULL);
        ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYGLOBALDATA, MSGFLT_ALLOW,
                                    NULL);
    }
    ...
    if (!window->monitor) {
        ...
        if (wndconfig->maximized && !wndconfig->decorated) {
            ...
            SetWindowPos(window->win32.handle, HWND_TOP, mi.rcWork.left, mi.rcWork.top,
                         ...
                         mi.rcWork.bottom - mi.rcWork.top, SWP_NOACTIVATE | SWP_NOZORDER);
        }
    }
...
void _glfwSetWindowIconWin32(_GLFWwindow* window, int count, const GLFWimage* images) {
    ...
    if (count) {
        const GLFWimage* bigImage = chooseImage(
            count, images, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON));
        const GLFWimage* smallImage = chooseImage(
            count, images, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
...
void _glfwSetWindowSizeLimitsWin32(_GLFWwindow* window, int minwidth, int minheight,
...
void _glfwSetWindowMonitorWin32(_GLFWwindow* window, _GLFWmonitor* monitor, int xpos,
...
GLFWbool _glfwCreateCursorWin32(_GLFWcursor* cursor, const GLFWimage* image, int xhot,
                                int yhot) {
...
GLFWbool _glfwCreateStandardCursorWin32(_GLFWcursor* cursor, int shape) {
    ...
    cursor->win32.handle = LoadImageW(NULL, MAKEINTRESOURCEW(id), IMAGE_CURSOR, 0, 0,
                                      LR_DEFAULTSIZE | LR_SHARED);
